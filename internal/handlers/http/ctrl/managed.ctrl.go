package ctrl

import (
	"os"
	"strings"
	"time"

	model_config "xi/internal/models/config"
	"xi/internal/web"
	"xi/internal/config/cfg"
	"xi/pkg/util"
	"xi/internal/infra/store"

	"github.com/gin-gonic/gin"
)

type ManagedCtrl struct {
	// mu   sync.RWMutex
	// once sync.Once
}

var Managed = &ManagedCtrl{}

// All cfg.Web.Pages[@].Route are added automatically for mode={"", "managed"}
func (m *ManagedCtrl) RouterCore(r *gin.Engine) {
	for _, p := range cfg.Web.Pages {
		if p != nil && (p.Ctrl.Mode == "" || strings.ToLower(p.Ctrl.Mode) == "managed") {

			// determine the HTTP method, default to GET
			method := "GET"
			if p.Ctrl.Method != "" {
				method = strings.ToUpper(p.Ctrl.Method)
			}

			// register route based on method
			switch method {
			case "GET":
				r.GET(p.Route, func(c *gin.Context) { web.Web.Page(c, p) })
			case "POST":
				r.POST(p.Route, func(c *gin.Context) { web.Web.Page(c, p) })
			case "PUT":
				r.PUT(p.Route, func(c *gin.Context) { web.Web.Page(c, p) })
			case "PATCH":
				r.PATCH(p.Route, func(c *gin.Context) { web.Web.Page(c, p) })
			case "DELETE":
				r.DELETE(p.Route, func(c *gin.Context) { web.Web.Page(c, p) })
			case "OPTIONS":
				r.OPTIONS(p.Route, func(c *gin.Context) { web.Web.Page(c, p) })
			case "HEAD":
				r.HEAD(p.Route, func(c *gin.Context) { routeMethodHead(c, p) })
			case "PURGE":
				// Gin doesn't have built-in PURGE, use Handle
				r.Handle("PURGE", p.Route, func(c *gin.Context) { web.Web.Page(c, p) })
			default:
				// fallback to GET if unknown
				r.GET(p.Route, func(c *gin.Context) { web.Web.Page(c, p) })
			}
		}
	}

}
func routeMethodHead(c *gin.Context, p *model_config.WebPage) {

	if len(p.Ctrl.Header) == 0 {
		// Add default headers to the slice if empty
		p.Ctrl.Header = append(
			p.Ctrl.Header,
			[]model_config.WebCtrlHeader{
				{Key: "Location", Value: cfg.Org.URL},
				{Key: "X-Endpoint-Status", Value: "Available"},
			}...,
		)
	}
	for _, header := range p.Ctrl.Header {
		c.Header(header.Key, header.Value)
	}
	if p.Ctrl.StatusCode == 0 {
		p.Ctrl.StatusCode = 200 // Default to 200 OK if not set
	}
	c.Status(p.Ctrl.StatusCode)
}

// Sitemap
func (m *ManagedCtrl) SitemapCore(c *gin.Context) (any, error) {
	rdbKey := c.Request.URL.Path + ".managed"
	urls := []model_config.MetaSitemap{}

	// Try cache
	if err := store.Rdb.GetJson(rdbKey, &urls); err == nil {
		return urls, nil
	}

	for _, p := range cfg.Web.Pages {

		if p == nil || p.Route == "" || !(p.Ctrl.Mode == "" || strings.ToLower(p.Ctrl.Mode) == "managed") {
			continue
		}

		// default fallbacks
		lastMod := func() string {
			// Case 1: Use UpdatedAt if present
			if p.Meta.UpdatedAt != nil {
				return p.Meta.UpdatedAt.Format("2006-01-02")
			}

			// Case 2: If page is file-based, check its mod time
			if p.Ctrl.Render == "file" && p.Content.File != "" {
				if fi, err := os.Stat(p.Content.File); err == nil {
					return fi.ModTime().Format("2006-01-02")
				}
			}

			return time.Now().Format("2006-01-02")
		}()
		changeFreq := "monthly"
		priority := "0.5"

		// If meta info available, override
		urls = append(urls, model_config.MetaSitemap{
			Loc:        util.Str.Fallback(p.Meta.Canonical, cfg.Org.URL+p.Route),
			LastMod:    util.Str.Fallback(p.Meta.Sitemap.LastMod, lastMod),
			ChangeFreq: util.Str.Fallback(p.Meta.Sitemap.ChangeFreq, changeFreq),
			Priority:   util.Str.Fallback(p.Meta.Sitemap.Priority, priority),
		})
	}

	// Cache
	go func() { store.Rdb.SetJson(rdbKey, urls, 15*time.Minute) }()
	return urls, nil
}
