...
func (mh *MetaHtml) Gen(p *model_config.Page) template.HTML {
	mh.p = p
	mh.m = &p.Meta
	m := &p.Meta
	...
	add(&b, `<meta property="og:image:alt" content="%s">`, util.Str.NotEmptyThen(m.Img.URL, util.Str.Fallback(m.Img.Alt, m.Title)))
...
func (mh *MetaHtml) LdJSON(m *model_config.Meta) []byte {
	// computed
	title := mh.Title(m)
...
func (mh *MetaHtml) Title(m *model_config.Meta) string {
	base := m.Title
...
func (mh *MetaHtml) Type(m *model_config.Meta) {
	if mh.m.Type == "" {
		mh.m.Type = "WebSite"
	}
