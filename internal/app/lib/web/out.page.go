...
func (v *WebLib) Page(c *gin.Context, p *model_config.Page) bool {
	...
	case "raw":
		...
		default:
			c.Status(http.StatusInternalServerError)
			log.Error().Caller().Str("type", fmt.Sprintf("%T", v)).Str("Page", c.Request.URL.Path).Msg("web Page, Unsupported content type in p.Rt[\"content\"]")
			return false
		}
