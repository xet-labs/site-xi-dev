...
var (
	Conf = &ConfLib{
		FilesDefault: []string{"internal/data/config/config.json"},
		Hooks:        &hook.Hook{},
	}
...
func init() {
	...
	loadConfigs := func(path string) []string {
		files, err := util.File.GetWithExt(".json", path)
		if err != nil {
			log.Fatal().Err(err).Msgf("Config error accessing %s", path)
			return []string{}
		}
		return util.Str.UniqueSort(files)
	}

	// Load internal then user configs
	Conf.FilesDefault = append(
		loadConfigs("internal/app/config"),
		loadConfigs("config")...,
	)
...
func (c *ConfLib) InitCore(filePath ...string) error {
	...
	for i, path := range c.Files {
		...
		noConfigKillSwitch := func() {
			...
			if !c.hasInitialized && len(newFilesLoaded) == 0 && i == len(c.Files)-1 {
				log.Fatal().Str("files", strings.Join(c.Files, ", ")).
	...
	log.Info().Str("files", strings.Join(newFilesLoaded, ", ")).Msgf("Config %s", okStatus)
