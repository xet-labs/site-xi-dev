...
type MetaHtml struct {
	p *model_config.WebPage
	m *model_config.WebMeta
}
...
func (mh *MetaHtml) Gen(p *model_config.WebPage) template.HTML {
	mh.p = p
	mh.m = &p.Meta
	m := &p.Meta
	...
	add(&b, `<meta property="og:image:alt" content="%s">`, util.Str.IfNotEmpty(m.Img.URL, util.Str.Fallback(m.Img.Alt, m.Title)))

	// --- Twitter ---
	add(&b, `<meta name="twitter:card" content="%s">`, util.Str.IfNotEmpty(m.Img.URL, "summary_large_image"))
	...
	add(&b, `<meta name="twitter:image:alt" content="%s">`, util.Str.IfNotEmpty(m.Img.URL, util.Str.Fallback(m.Img.Alt, m.Title)))
	...
	if isArticle(m.Type) {
		add(&b, `<meta name="twitter:label1" content="%s">`, util.Str.IfNotEmpty(m.Author.Name, "Written by"))
		add(&b, `<meta name="twitter:data1" content="%s">`, "@"+m.Author.Name)

		add(&b, `<meta name="twitter:label2" content="%s">`, util.Str.IfNotEmpty(m.Category, "Category"))
		add(&b, `<meta name="twitter:data2" content="%s">`, m.Category)

		add(&b, `<meta name="twitter:label3" content="%s">`, util.StrIfPtrNotNil(m.CreatedAt, "Published on"))
		add(&b, `<meta name="twitter:data3" content="%s">`, m.CreatedAt.UTC().Format(time.RFC3339Nano))

		add(&b, `<meta name="twitter:label4" content="%s">`, util.Str.IfNotEmpty(m.ReadingTime, "Reading time"))
...
func (mh *MetaHtml) LdJSON(m *model_config.WebMeta) []byte {
	// computed
	title := mh.Title(m)
...
func (mh *MetaHtml) Title(m *model_config.WebMeta) string {
	base := m.Title
	...
	if mh.m.TitleOrgSuffixInc != nil && *mh.m.TitleOrgSuffixInc {
		if mh.p.Org.Name != "" {
			return base + mh.m.TitleOrgSuffixSep + mh.p.Org.Name
		}
	}
...
func (mh *MetaHtml) Type(m *model_config.WebMeta) {
	if mh.m.Type == "" {
		mh.m.Type = "WebSite"
	}
	switch strings.ToLower(util.Str.Fallback(mh.m.OG.Type, mh.m.Type)) {
	case "article", "blogposting", "newsarticle":
		mh.m.OG.Type = "article"
	case "product":
		mh.m.OG.Type = "product"
	case "profile":
		mh.m.OG.Type = "profile"
	default:
		mh.m.OG.Type = "website"
	}
}
