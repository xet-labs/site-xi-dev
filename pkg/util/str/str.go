...
func (s *StrLib) IfNotEmpty(str, val string) string {
	if str != "" {
		return val
	}
	return ""
...
func (s *StrLib) IfNotEmptyElse(str, val, fallback string) string {
	if str != "" {
		return val
	}
	return fallback
...
func (u *StrLib) QuoteSlice(items []string) string {
	return strings.Join(u.StringSlice(items), ", ")
}

func (u *StrLib) StringSlice(items []string) []string {
