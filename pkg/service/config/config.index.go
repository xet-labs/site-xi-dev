...
var (
	Config = &ConfigService{
		DirDefault: []string{"pkg/config", "config"},
		Hooks:      &hook.Hook{},
	}
...
func (c *ConfigService) Init(filePath ...string) {
	c.once.Do(func() {
...
func (c *ConfigService) InitCore(files ...string) error {
	env.Env.Init()
	c.mu.Lock()
	defer c.mu.Unlock()
...
// Load Config Files
func (c *ConfigService) LoadConfigsPath() {
...
func (c *ConfigService) Error(err error) {
	panic("unimplemented")
}

func (c *ConfigService) preProcess(rawJson []byte) ([]byte, model_config.Config, error) {
...
func (c *ConfigService) resolveJsonEnv(input string) string {
...
func (c *ConfigService) cleanJson(input string) string {
	// Remove "__REMOVE__"
...
func (c *ConfigService) resolveJsonVars(input string) (string, error) {
	var data any
...
func (c *ConfigService) MergeConf(rawConf *map[string]any) error {
...
func (c *ConfigService) Daemon() error {
	if !cfg.App.ConfigHotReloadSig && c.watch != nil {
	...
	log.Info().Msg("config hot-reload enabled")
	c.watch = watcher
