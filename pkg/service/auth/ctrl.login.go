...
func (h *AuthCtrl) Login(c *gin.Context) {
	...
	if err := h.S.DB.Where("username = ? OR email = ?", req.Username, req.Username).First(&user).Error; err != nil {
	...
	access, err := h.S.GenAccessToken(fmt.Sprint(user.ID), []string{"default"}, h.S.AccessTTL)
	if err != nil {
	...
	ip := c.ClientIP()
	rawRefresh, _, err := h.S.GenRefreshTokenRecord(user.ID, ua, ip)
	if err != nil {
	...
	cookieName := "refresh"
	c.SetCookie(cookieName, rawRefresh, maxAge, "/", h.S.CookieDomain, h.S.CookieSecure, true) // last true = HttpOnly
